...
const _default: Messages<ProfileMessage> = {
    zh_CN: {
        p1: appMessages.zh_CN.name + '是一款开源、免费、用户友好的，用于统计上网时间的浏览器扩展。您可以在 {github} 上查阅它的源代码。',
        p2: '这个页面将会告诉您如何使用它，以及相关的隐私政策。'
    },
    zh_TW: {
        p1: appMessages.zh_TW.name + '是一款開源、免費、用戶友好的，用於統計上網時間的瀏覽器擴展。您可以在 {github} 上查閱它的源代碼。',
    ...
    ja: {
        p1: appMessages.ja.name + 'は、オンラインで費やした時間をカウントするための、オープン ソースで無料のユーザー フレンドリーなブラウザ拡張機能です。 {github} でソース コードを確認できます。 ',
