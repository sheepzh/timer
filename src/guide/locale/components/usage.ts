...
const messages: Messages<UsageMessage> = {
    zh_CN: {
        quickstart: {
        ...
        background: {
            ...
            l1: '1. 您可以右击扩展的图标，在弹出的菜单中点击【' + chromeBase.zh_CN.allFunction + '】。',
            l2: '2. 您在图标弹出页的下方也可以找到【' + chromeBase.zh_CN.allFunction + '】链接，同样点击它即可。',
    ...
    zh_TW: {
        ...
        background: {
            ...
            l1: '1. 您可以右擊擴展的圖標，在彈出的菜單中點擊【' + chromeBase.zh_TW.allFunction + '】。',
            l2: '2. 您在圖標彈出頁的下方也可以找到【' + chromeBase.zh_TW.allFunction + '】鏈接，同樣點擊它即可。',
    ...
    en: {
        ...
        background: {
            p1: 'Based on icons, the extension provides a more convenient way to view data. \
                    But if you want to experience its full functionality, you need to visit {background} of the extension. \
                    There are two ways to enter the background page:',
            l1: '1. You can right-click the icon of the extension, and click [' + chromeBase.en.allFunction + '] in the pop-up menu.',
            l2: '2. You can also find the [' + chromeBase.en.allFunction + '] link at the bottom of the icon popup page, just click it.',
        ...
        advanced: {
            p1: 'The core function of this extension is to count your browsing behavior on different websites. \
                    In addition, it also provides many advanced functions to meet your more needs. \
                    Of course, you can find all the functions in the background page.',
            l1: '1. It can analyze the trend of your visiting the same website over a period of time, and display it in a line chart.',
            l2: '2. It can count your surfing frequency in different time periods every day, and display it in a histogram. \
                    The data is site-agnostic and has a minimum statistical granularity of 15 minutes.',
    ...
    ja: {
        quickstart: {
            ...
            l1: '1. ブラウザの右上隅にある拡張機能のアイコンを修正します。具体的な操作方法はブラウザによって異なります。 \
                この手順は、拡張機能の通常の操作には影響しませんが、インタラクティブなエクスペリエンスを大幅に向上させます。',
            l2: '2. 任意の Web サイトを開いて数秒間ブラウジングすると、右上隅のアイコンに数字がジャンプしていることがわかります。\
                これは、現在の Web サイトの閲覧に今日どれだけの時間を費やしたかを示しています。',
            l3: '3. 拡張機能のアイコンをクリックすると、ページがポップアップし、今日または最近のインターネット データが表示されます。',
            p2: 'なお、継続時間データはリアルタイムでカウントされるため、拡張機能をインストールする前の閲覧履歴は記録されません。',
        },
        background: {
            p1: 'アイコンに基づいて、拡張機能はデータを表示するためのより便利な方法を提供します。 \
                ただし、完全な機能を体験したい場合は、拡張 {background} にアクセスする必要があります。 \
                バックグラウンド ページに入る方法は 2 つあります。',
            l1: '1. 拡張機能のアイコンを右クリックし、ポップアップ メニューで [' + chromeBase.ja.allFunction + '] をクリックします。',
            l2: '2. また、アイコン ポップアップ ページの下部に [' + chromeBase.ja.allFunction + '] リンクがあり、それをクリックするだけです。',
            p2: 'ポップアップ ページと背景ページは、この拡張機能の主な対話方法であり、それらを開く方法を理解すれば、完全に使用できます。',
            backgroundPage: '背景ページ',
        },
        advanced: {
            p1: 'この拡張機能の主な機能は、さまざまな Web サイトでの閲覧行動をカウントすることです。 \
                さらに、より多くのニーズを満たすために多くの高度な機能も提供します。 \
                もちろん、バックグラウンド ページですべての機能を見つけることができます。',
            ...
            l8: '8. Github Gist をクラウドとして使用して、複数のブラウザーのデータを保存し、集約されたクエリを実行することをサポートします。 \
                少なくとも gist 権限を持つトークンを準備する必要があります。',
        }
    }
}
