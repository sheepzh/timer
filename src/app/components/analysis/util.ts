 ...
 * @param fromatter formatter
 * @returns text or '-'
 */
export function computeRingText(ring: RingValue, fromatter?: ValueFormatter): string {
    if (!ring) {
        return '-'
    }
    ...
    let result = fromatter ? fromatter(delta) : delta?.toString()
