...
import { tooptipDot } from "@app/util/echarts"
...
function optionOf(lastPeriodItems: Row[], thisPeriodItems: Row[]): EcOption {
    ...
    const color1 = getCssVariable('--echart-bar-compare-color-1')
    const color2 = getCssVariable('--echart-bar-compare-color-2')
    const incColor = getCssVariable('--echart-increase-color')
    const decColor = getCssVariable('--echart-decrease-color')
    return {
        title: {
            ...BASE_TITLE_OPTION,
        ...
        tooltip: {
            ...
            formatter(params: TopLevelFormatterParams) {
                ...
                const lastStr = `${tooptipDot(lastColor as string)}&emsp;${cvt2LocaleTime(lastDate)}&emsp;<b>${formatPeriodCommon(lastVal)}</b>`
                let thisStr = `${tooptipDot(thisColor as string)}&emsp;${cvt2LocaleTime(thisDate)}&emsp;<b>${formatPeriodCommon(thisVal)}</b>`
                if (lastVal) {
